...
data class AppQuickActionsItem(
  val type: String,
  val title: String,
  ...
  companion object {
    ...
    fun fromReadableMap(map: ReadableMap): AppQuickActionsItem? {
      ...
      return AppQuickActionsItem(type, title, shortTitle ?: title, iconName, jsonObject)
    }

    @TargetApi(25)
    fun fromPersistentBundle(bundle: PersistableBundle): AppQuickActionsItem? {
    ...
    fun toWritableArray(items: List<AppQuickActionsItem>): WritableArray {
